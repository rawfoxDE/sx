...
static APTR BT_N,BT_Name,BT_Loca,BT_Act,BT_I;
static APTR LI_Nodes, LV_Nodes;
...
Object *app, *muiwin, *listview, *list;
...
void UpdateDisplay(int i, char sel)
{
	...
	SetBPen(rp, 0);
     ...
     DoMethod(LV_Nodes, MUIM_List_InsertSingle, buf, MUIV_List_Insert_Top);
     DoMethod(LV_Nodes, MUIM_List_Sort);
     DoMethod(LV_Nodes, MUIM_List_Redraw, MUIV_List_Redraw_All);
...
void MyWin(void)
{
	...
	app = ApplicationObject,
		...
		MUIA_Application_Window, muiwin = WindowObject,
		MUIA_Window_Title,		  "SX-MCP",
		MUIA_Window_ID,		  'MCPW',
		WindowContents, VGroup,

               Child, ColGroup(5),
                              
                    Child, BT_N    = SimpleButton("N"),
                     MUIA_Weight, 16,
                    Child, BT_Name = SimpleButton("Name / Handle"),
                    Child, BT_Loca = SimpleButton("Location / Group"),
                    Child, BT_Act  = SimpleButton("Action / Door"),
                    Child, BT_I    = SimpleButton("I"),
                     MUIA_Weight, 16,
               End,

               Child, HGroup,
                              
		          Child, LV_Nodes = ListviewObject,
			     MUIA_Listview_List, LI_Nodes = ListObject,
	...
	if(app)
	{
		set(muiwin,MUIA_Window_Open, TRUE);
          ...
          DoMethod(muiwin, MUIM_Notify, MUIA_Window_CloseRequest, TRUE, muiwin, 3, MUIM_Set, MUIA_Window_Open, FALSE);
	     
          DoMethod(LV_Nodes, MUIM_List_Sort);
     

		/* Add an extra item... */
		DoMethod(LV_Nodes, MUIM_List_InsertSingle, "Sneaker Pimps", MUIV_List_Insert_Sorted);
