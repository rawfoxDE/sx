...
#define ID_CM_TELNET     30
#define ID_CM_IDLE       31
#define ID_CM_MULTI      32
#define ID_CM_CLOSED     33

#define ID_CM_RTS7       40
#define ID_CM_SHARED     41
#define ID_CM_DSR        42
...
char MenuEntries[256][52],ProtoEntries[66][256];
...
void LoadMain(void);
void LoadNodePrefs(void);

void LoadDoorsConfig(void);
...
static APTR protoTEXT,serTEXT, doorTEXT, thisTEXT;        
...
long mEFF_pr;
char mNAME_pr[24],mDEV_pr[30],mINIT_pr[64];
const char mLETTER_pr;
...
char mSERDEV_sm[32],mSERUNIT_sm[3], mSERBAUD_sm[6], mDCERATE_sm[6];
char mCONNECT_sm[24],mRING_sm[16],mINIT_sm[48],mANSWER_sm[16];
...
static APTR BT_Cancel_sm, BT_Save_sm;

//This Node window

char mSYSPW_tn[16], mNUPE_tn[16],mFCHECKDIR_tn[42];
...
static APTR BT_ArcTyp,BT_Protocol,BT_SerMod,BT_Doors,BT_This,BT_MCPConf,BT_UserEd,BT_Quit;
...
char mCHAT_door[52],mMSGED_door[52],mMSGRE_door[52],mFRONT_door[52],mNEWF_door[52];
ULONG mCHATTYP_door, mMSGEDTYP_door, mMSGRETYP_door,mFRONTTYP_door,mNEWFTYP_door;
...
static APTR BT_Cancel_door, BT_Save_door;
...
//Save Serial Preafs
void SAVEDS ASM SaveSerialPrefsFunc(void)
{
     BPTR fhs;          
     fhs = Open(buf, MODE_NEWFILE);
     if(fhs)
     {
        mysprintf(buf,"%sSerial.DAT", path);               
        strcpy(Serial.device, mSERDEV_sm);          
        Serial.unit    = atoi(mSERUNIT_sm);     
        Serial.minrate = atoi(mSERBAUD_sm);     
        Serial.dcerate = atoi(mDCERATE_sm);         
        strcpy(Serial.connect, mCONNECT_sm);     
        strcpy(Serial.ring   , mRING_sm );     
        strcpy(Serial.answer , mANSWER_sm);     
        strcpy(Serial.initstr, mINIT_sm);  
        
        get(CM_RTS7, MUIA_Selected, &selected);
        if(selected) Serial.sevenwire=1; else Serial.sevenwire=0;
        get(CM_SHARED, MUIA_Selected, &selected);
        if(selected) Serial.shared=1; else Serial.shared=0;
        get(CM_DSR, MUIA_Selected, &selected);
        if(selected) Serial.flags|=1<<0; else Serial.flags=~1<<0;
                 
        Write(fhs, &Serial, sizeof(struct SerialStruct));
        Close(fhs);
        MUI_Request(AP_Demo, WI_Serial, 0, NULL, "OK", "Config was successfull saved !");
	
     } else MUI_Request(AP_Demo, WI_Master, 0, NULL, "hmm", "Fail to save Config !");	     
     DoMethod(WI_Serial,MUIM_Set,MUIA_Window_Open,FALSE);         
            
} static struct Hook SaveSerialPrefsHook={ {NULL,NULL}, (void *)SaveSerialPrefsFunc, NULL,NULL };

// Save Doors Prefs
void SAVEDS ASM SaveDoorsFunc(void)
{
     BPTR fhd;          
     mysprintf(buf, "SX:Node%ld/Prefs/Doors.DAT",node);      
     fhd = Open(buf, MODE_NEWFILE);
     if(fhd)
     {     
        strncpy(doors.chatdoor, mCHAT_door,52);        
        get(CY_CHAT, MUIA_Cycle_Active, &mCHATTYP_door);
        doors.chattype = mCHATTYP_door+1;        
        strncpy(doors.msgdoor ,mMSGED_door,52);
        get(CY_MSGED, MUIA_Cycle_Active, &mMSGEDTYP_door);
        doors.msgtype = mMSGEDTYP_door+1;        
        strncpy(doors.frontdoor, mFRONT_door,52);
        get(CY_FRONT, MUIA_Cycle_Active, &mFRONTTYP_door);
        doors.fronttype = mFRONTTYP_door+1;        
        strncpy(doors.msgdoorreply, mMSGRE_door, 52);
        get(CY_MSGRE, MUIA_Cycle_Active, &mMSGRETYP_door);
        doors.msgdoorreplytype = mMSGRETYP_door+1;        
        strncpy(doors.nfdoor , mNEWF_door, 52);
        get(CY_NEWF, MUIA_Cycle_Active, &mNEWFTYP_door);
        doors.nftype = mNEWFTYP_door+1;        
        Write(fhd, &doors, sizeof(struct DoorsStruct));
        Close(fhd);     
        MUI_Request(AP_Demo, WI_Doors, 0, NULL, "OK", "Config was successfull saved !");	
     } else MUI_Request(AP_Demo, WI_Master, 0, NULL, "hmm", "Fail to save Config !");	     
     DoMethod(WI_Doors,MUIM_Set,MUIA_Window_Open,FALSE); 

} static struct Hook SaveDoorsHook={ {NULL,NULL}, (void *)SaveDoorsFunc, NULL,NULL };

// Save Nodes Prefs
void SAVEDS ASM SaveNodesFunc(void)
{
     BPTR fhn;          
     mysprintf(buf, "SX:Node%ld/Prefs/NodePrefs.DAT",node);      
     fhn = Open(buf, MODE_NEWFILE);
     if(fhn)
     {
          get(CM_TELNET, MUIA_Selected, &selected);
          if(selected) nodeprefs.flags|=1<<0; else nodeprefs.flags=~1<<0;
          get(CM_CLOSED, MUIA_Selected, &selected);
          if(selected) nodeprefs.flags|=1<<1; else nodeprefs.flags=~1<<0;
          get(CM_MULTI, MUIA_Selected, &selected);
          if(selected) nodeprefs.flags|=1<<2; else nodeprefs.flags=~1<<2;
          //get(CM_IDLE, MUIA_Selected, &selected);
          //if(selected) nodeprefs.flags|=1<<3; else nodeprefs.flags=~1<<3;            
                  
          strcpy(nodeprefs.systempass ,mSYSPW_tn);
          strcpy(nodeprefs.newuserpass,mNUPE_tn);
          strcpy(nodeprefs.fcheck     ,mFCHECKDIR_tn);          
               
          Write(fhn, &nodeprefs, sizeof(struct NodeStruct));
          Close(fhn);
          MUI_Request(AP_Demo, WI_This, 0, NULL, "OK", "Config was successfull saved !");
	          
     } else MUI_Request(AP_Demo, WI_Master, 0, NULL, "hmm", "Fail to save Config !");
	DoMethod(WI_This,MUIM_Set,MUIA_Window_Open,FALSE);
     
} static struct Hook SaveNodesHook={ {NULL,NULL}, (void *)SaveNodesFunc, NULL,NULL };
...
void SAVEDS ASM SaveMainFunc(void)
{
	...
	if(fh)
     {                                                           
          strcpy(Main.BBSName  ,mBBSName);
	     strcpy(Main.Sysop    ,mSYSOP);
          strcpy(Main.BBSLoc   ,mLOC);
          strcpy(Main.USERPath ,mPATH);                
          strncpy(Main.DNPath  ,mDPATH, 64);                
          strncpy(Main.ULPath  ,mUPATH, 64);                
          Main.nodes = mNODES;
		...
		Close(fh);
	     MUI_Request(AP_Demo, WI_Main, 0, NULL, "OK", "Config was successfull saved !");
...
// Load Protocols  
void SAVEDS ASM ProtoListFunc(void)
{              
        BPTR fhp; 
        ...
        mysprintf(buf,"%sProtocols.DAT", path);
        fhp = Open(buf, MODE_OLDFILE);        
        
        DoMethod(LI_PROTO, MUIM_List_Clear);
        
        if(!fhp)
        {
             maxproto=1;
             
             strcpy(proto.name, "*new*");
		   DoMethod(LI_PROTO, MUIM_List_InsertSingle, proto.name ,1,MUIV_List_Insert_Sorted);
        }else   
        {
             while(fhp)
             {
               i++;             
               if(i==64) { maxproto=i; break; }
             
               if(!Read(fhp, &proto, sizeof(struct ProtoStruct))) 
               { 
                    maxproto = i;
                    break;
               }
                         
               strcpy(ProtoEntries[i], proto.name);
                        
               DoMethod(LI_PROTO, MUIM_List_InsertSingle, ProtoEntries[i] ,1,MUIV_List_Insert_Sorted);
             }
             Close(fhp);
        }
        
        //DoMethod(LI_PROTO, MUIM_List_Insert, MUIA_List_SourceArray,i, ProtoEntries);
              
        DoMethod(LI_PROTO, MUIM_List_Sort);
        
                           
                        
} static struct Hook ProtoListHook={ {NULL,NULL}, (void *)ProtoListFunc, NULL,NULL };


//Load Protocol Prefs
void SAVEDS ASM LoadProtoFunc(void)
{       
        LoadProtocol();
} static struct Hook LoadProtoHook={ {NULL,NULL}, (void *)LoadProtoFunc, NULL,NULL };


// Load Serial Prefs  
void SAVEDS ASM LoadSerialFunc(void)
{       
        LoadSerial();
} static struct Hook LoadSerialHook={ {NULL,NULL}, (void *)LoadSerialFunc, NULL,NULL };


// Load Doors Prefs  
void SAVEDS ASM LoadDoorsFunc(void)
{       
        LoadDoorsConfig();
} static struct Hook LoadDoorsHook={ {NULL,NULL}, (void *)LoadDoorsFunc, NULL,NULL };
...
{       
        set(SL_NODE, MUIA_Disabled, TRUE);
        LoadNodePrefs();
...
{        
        mysprintf(path, "SX:Node%ld/Prefs/",node);
        LoadNodePrefs();
} static struct Hook SetNodePathHook={ {NULL,NULL}, (void *)SetNodePathFunc, NULL,NULL };


// Load User Editor 
void SAVEDS ASM LoadUserEdFunc(void)
{        
        mysprintf(buf,"c:run >NIL: SX:Utils/UserEd");
        Execute(buf,0,0);
                
} static struct Hook LoadUserEdHook={ {NULL,NULL}, (void *)LoadUserEdFunc, NULL,NULL };
		...
		MUIA_Application_Menustrip     , MUI_MakeObject(MUIO_MenustripNM,Menu,0),
                ...
                SubWindow,      //                   Transfer Protocols WINDOW
                        WI_PCol = DemoWindow("SX-Config Protocols",MAKE_ID('S','X','P','R'),NULL),                               
                           ...
                           Child,protoTEXT = TextObject, TextFrame,
                           ...
                           Child, HGroup,                                                                   
                                ...
                                Child, VGroup, GroupFrameT("Setup"),                                                                      
                                    ...
                                    Child, HGroup,MUIA_Group_Columns, 2, 
                                                ...
                                                Child,POP_LIB_pr = PopaslObject,
                                                  ...
                                                  MUIA_Popstring_String, KeyString(mDEV_pr,30,'l'),//  Main.ULPath,256,'s'),
                                                  MUIA_Popstring_Button,PopButton(MUII_PopDrawer), 
                                                  ASLFR_TitleText,"Select Protocol Library...",
                                                  ASLFR_DrawersOnly, FALSE,

                                                  ASLFR_InitialDrawer,"LIBS:",
                                                  ASLFR_InitialPattern, "#?.library",
                                    End,                                            
			...
			WI_Serial = DemoWindow("SX-Config Serial/Modem",MAKE_ID('S','E','R','M'),NULL), 
                           ...
                           Child,serTEXT = TextObject, TextFrame,
                                          MUIA_Background, MUII_SHINEBACK,
                                          MUIA_Text_SetVMax , FALSE,
                                          MUIA_Text_PreParse, "\33c",
                                          End,
                           ...
                           Child, VGroup,     
                                Child, VGroup, GroupFrameT("Device"),
                                        ...
                                        Child,POP_SERDEV = PopaslObject,
                                                  MUIA_Popasl_Type, ASL_FileRequest, 
                                                  MUIA_Popstring_String, KeyString(mSERDEV_sm,32,'s'),
                                                  MUIA_Popstring_Button,PopButton(MUII_PopDrawer), 
                                ...
                                Child, HGroup,                   
                                Child, BT_Save_sm   = SimpleButton("_Save"),
                                Child, BT_Cancel_sm = SimpleButton("_Cancel"),
			        End,                             
                           End,
                        End,
                ...
                SubWindow, //                               This Node WiNDOW
                        WI_This = DemoWindow("SX-Config This Node",MAKE_ID('T','N','O','D'),NULL),
                          ...
                          Child,thisTEXT = TextObject, TextFrame,
                                             MUIA_Background, MUII_SHINEBACK,
                                             MUIA_Text_SetVMax , FALSE,
                                             MUIA_Text_PreParse, "\33c",
                                             End,
			...
			WI_Doors = DemoWindow("SX-Config Doors",MAKE_ID('D','O','O','R'),NULL),
                                ...
                                Child,doorTEXT = TextObject, TextFrame,
                                             MUIA_Background, MUII_SHINEBACK,
                                             MUIA_Text_SetVMax , FALSE,
                                             MUIA_Text_PreParse, "\33c",
                                             End,
                                ...
                                Child, HGroup, 
                                        Child, BT_Save_door   = SimpleButton("_Save"),
                                        Child, BT_Cancel_door = SimpleButton("_Cancel"),
			  	        End,
		        ...
		        WI_Main = DemoWindow("SX-Config Main",MAKE_ID('M','C','F','G'),FALSE),
                           	...
                           	Child, VGroup, GroupFrameT("Main Settings"),
                                ...
                                Child,POP_DPATH = PopaslObject, 
                                                  MUIA_Popstring_String,KeyString(mDPATH,64,'d'),
                                                  MUIA_Popstring_Button,PopButton(MUII_PopDrawer), 
                                ...
                                Child,POP_UPATH = PopaslObject, 
                                                  MUIA_Popstring_String, KeyString(mUPATH,64,'u'),
                                                  MUIA_Popstring_Button,PopButton(MUII_PopDrawer), 
		...
		SubWindow,                                 // SX-CONFIG WINDOW
			...
			WindowContents, VGroup,
				Child, TextObject, GroupFrame, MUIA_Background, MUII_FILL, MUIA_Text_Contents, "\033c\033bSX-Config\033n\n \33bM\33nain\33bC\33nonfiguration\33bI\33nnterface",  End,		
					...
					Child, BT_MCPConf   = SimpleButton("MCP_Config"),
                         Child, BT_UserEd    = SimpleButton("GUI User Editor"),
...
//----------------------------------------------------------------------sx-config 
      ...
      //Conferences Button
        ...
        set(BT_Confs, MUIA_Disabled,TRUE);
      ...
      //Internal Map Button
        ...
        set(BT_IntMap, MUIA_Disabled, TRUE);

      //AccessSets Button
        set(BT_AxxSet,MUIA_Disabled, TRUE);
        
      //CronEvents Button
        set(BT_Cron, MUIA_Disabled, TRUE);
        
      //Network settings Button
        set(BT_NetSet, MUIA_Disabled, TRUE);
        
      //MCP Config Button
        set(BT_MCPConf, MUIA_Disabled, TRUE);
      
      //UserEditor Button
        DoMethod(BT_UserEd,MUIM_Notify,MUIA_Pressed,FALSE,
                 BT_UserEd ,2,MUIM_CallHook, &LoadUserEdHook);
      
      //Local Display Button
        set(BT_LocDis, MUIA_Disabled, TRUE);
        
      //Display Modes Button
        set(BT_DisMod, MUIA_Disabled, TRUE);
          
      //Archive Types Button
        DoMethod(BT_ArcTyp  ,MUIM_Notify,MUIA_Pressed,FALSE,
        WI_Arc   ,3,MUIM_Set,MUIA_Window_Open,TRUE);            
        set(BT_ArcTyp, MUIA_Disabled, TRUE);
              
      //Protocols window Button
        DoMethod(BT_Protocol   ,MUIM_Notify,MUIA_Pressed,TRUE,
                 BT_Protocol   ,2,MUIM_CallHook, &SetNodePathHook);
                 
        DoMethod(BT_Protocol   ,MUIM_Notify,MUIA_Pressed,FALSE,
                 BT_Protocol   ,2,MUIM_CallHook, &LoadProtoHook);                 
        //set(BT_Protocol, MUIA_Disabled, TRUE);
                 
      //Serial&Modem window Button
        DoMethod(BT_SerMod, MUIM_Notify, MUIA_Pressed,TRUE,
                 BT_SerMod,2,MUIM_CallHook, &SetNodePathHook);
        
        DoMethod(BT_SerMod,MUIM_Notify, MUIA_Pressed, FALSE,
                 BT_SerMod,2, MUIM_CallHook, &LoadSerialHook);               
                                                            
      //Doors window Button
        DoMethod(BT_Doors,MUIM_Notify, MUIA_Pressed, TRUE,
                 BT_Doors,2, MUIM_CallHook, &SetNodePathHook); 
        
        DoMethod(BT_Doors,MUIM_Notify, MUIA_Pressed, FALSE,
                 BT_Doors,2, MUIM_CallHook, &LoadDoorsHook);               
...
//ThisNode WINDOW
//-------------------------------------------------------------------This Node    
        DoMethod(BT_Cancel_tn ,MUIM_Notify, MUIA_Pressed,TRUE,
                 WI_This,2 ,MUIM_Set,MUIA_Window_Open,FALSE);
        
        DoMethod(WI_This, MUIM_Notify, MUIA_Window_Open, FALSE,              
                 SL_NODE,2 ,MUIM_Set,MUIA_Disabled, FALSE); 
        
        DoMethod(BT_Save_tn ,MUIM_Notify, MUIA_Pressed,TRUE,         
                 WI_This ,2,MUIM_CallHook, &SaveNodesHook);     
        
        DoMethod(WI_This ,MUIM_Notify, MUIA_Window_Open,FALSE,
                 SL_NODE,2 ,MUIM_Set, MUIA_Disabled, FALSE);        
                          
        DoMethod(ST_SYSPW_tn ,MUIM_Notify,MUIA_String_Acknowledge,MUIV_EveryTime,
                 MUIV_Notify_Self,3,MUIM_WriteString,MUIV_TriggerValue, mSYSPW_tn);        
        
        DoMethod(ST_NUPE_tn ,MUIM_Notify,MUIA_String_Acknowledge,MUIV_EveryTime,
                 MUIV_Notify_Self,3,MUIM_WriteString,MUIV_TriggerValue, mNUPE_tn);        
        
        DoMethod(ST_FCHECKDIR_tn ,MUIM_Notify,MUIA_String_Acknowledge,MUIV_EveryTime,
                 MUIV_Notify_Self,3,MUIM_WriteString,MUIV_TriggerValue, mFCHECKDIR_tn);
        
        DoMethod(CM_IDLE, MUIM_Set, MUIA_Disabled,TRUE);                                     
        
        
// Doors Replacements Window
//--------------------------------------------
        DoMethod(ST_CHAT_door, MUIM_Notify, MUIA_String_Acknowledge, MUIV_EveryTime,
                 MUIV_Notify_Self, 3, MUIM_WriteString, MUIV_TriggerValue, mCHAT_door);        
        
        DoMethod(ST_MSGED_door, MUIM_Notify, MUIA_String_Acknowledge, MUIV_EveryTime,
                 MUIV_Notify_Self, 3, MUIM_WriteString, MUIV_TriggerValue, mMSGED_door);
            
        DoMethod(ST_MSGRE_door, MUIM_Notify, MUIA_String_Acknowledge, MUIV_EveryTime,
                 MUIV_Notify_Self, 3, MUIM_WriteString, MUIV_TriggerValue, mMSGRE_door);
           
        DoMethod(ST_FRONT_door, MUIM_Notify, MUIA_String_Acknowledge, MUIV_EveryTime,
                 MUIV_Notify_Self, 3, MUIM_WriteString, MUIV_TriggerValue, mFRONT_door);
                     
        DoMethod(ST_NEWF_door, MUIM_Notify, MUIA_String_Acknowledge, MUIV_EveryTime,
                 MUIV_Notify_Self, 3, MUIM_WriteString, MUIV_TriggerValue, mNEWF_door);
           
        DoMethod(BT_Cancel_door ,MUIM_Notify, MUIA_Pressed,FALSE,
                 WI_Doors,2 ,MUIM_Set,MUIA_Window_Open,FALSE);
        
        DoMethod(BT_Save_door, MUIM_Notify, MUIA_Pressed, FALSE,
                 BT_Save_door,2,MUIM_CallHook, &SaveDoorsHook);
        DoMethod(WI_Doors, MUIM_Notify, MUIA_Window_Open, FALSE,
                 SL_NODE, 2, MUIM_Set, MUIA_Disabled, FALSE);
                
// SERIAL & Modem Window
//-----------------------------------------------
        DoMethod(POP_SERDEV,MUIM_Notify,MUIA_String_Acknowledge,MUIV_EveryTime,
                 MUIV_Notify_Self,3,MUIM_WriteString,MUIV_TriggerValue, mSERDEV_sm);        
        DoMethod(ST_SERUNIT, MUIM_Notify, MUIA_String_Acknowledge, MUIV_EveryTime,
                 MUIV_Notify_Self, 3, MUIM_WriteString, MUIV_TriggerValue, mSERUNIT_sm);
        DoMethod(ST_MINBAUD, MUIM_Notify, MUIA_String_Acknowledge, MUIV_EveryTime,
                 MUIV_Notify_Self, 3, MUIM_WriteString, MUIV_TriggerValue, mSERBAUD_sm);
        DoMethod(ST_DCE, MUIM_Notify, MUIA_String_Acknowledge, MUIV_EveryTime,
                 MUIV_Notify_Self, 3, MUIM_WriteString, MUIV_TriggerValue, mDCERATE_sm);
        DoMethod(ST_CONNECT, MUIM_Notify, MUIA_String_Acknowledge, MUIV_EveryTime,
                 MUIV_Notify_Self, 3, MUIM_WriteString, MUIV_TriggerValue, mCONNECT_sm);
        DoMethod(ST_RING, MUIM_Notify, MUIA_String_Acknowledge, MUIV_EveryTime,
                 MUIV_Notify_Self, 3, MUIM_WriteString, MUIV_TriggerValue, mRING_sm);        
        DoMethod(ST_ATA, MUIM_Notify, MUIA_String_Acknowledge, MUIV_EveryTime,
                 MUIV_Notify_Self, 3, MUIM_WriteString, MUIV_TriggerValue, mANSWER_sm);        
        DoMethod(ST_INIT, MUIM_Notify, MUIA_String_Acknowledge, MUIV_EveryTime,
                 MUIV_Notify_Self, 3, MUIM_WriteString, MUIV_TriggerValue, mINIT_sm);        
        DoMethod(BT_Save_sm, MUIM_Notify, MUIA_Pressed, FALSE,
                 BT_Save_sm, 2,MUIM_CallHook, &SaveSerialPrefsHook);
               
        DoMethod(BT_Cancel_sm, MUIM_Notify, MUIA_Pressed, FALSE,
                 WI_Serial,2, MUIM_Set, MUIA_Window_Open, FALSE);
        DoMethod(WI_Serial, MUIM_Notify, MUIA_Window_Open, FALSE,
                 SL_NODE, 2, MUIM_Set, MUIA_Disabled, FALSE);
        
// PROTOCOLS Window
//------------------------------------------------

        DoMethod(BT_Cancel_pr, MUIM_Notify, MUIA_Pressed, FALSE,
                 WI_PCol,2, MUIM_Set, MUIA_Window_Open, FALSE);
        
        DoMethod(WI_PCol, MUIM_Notify, MUIA_Window_Open, FALSE,              
                 SL_NODE,2 ,MUIM_Set,MUIA_Disabled, FALSE); 
        
// SET GADGET STERT VALUES HERE                
        ...
        set(ST_PATH      ,MUIA_String_Contents,Main.USERPath);
...
//QUIT and REMOVE 
    ...
    //MainLoop
	{
        ...
        DeleteFile("t:sxmenu.tmp");   
        for(j=0; j<i; i--) { MenuEntries[i][0]=0; }
        for(j=0; j<maxproto; maxproto--) { ProtoEntries[maxproto][0]=0; }
...
void LoadMain(void)
{
	...
	if(fh)
	{
     ...
     mysprintf(mBBSName, "%s", Main.BBSName);
     mysprintf(mSYSOP,"%s",Main.Sysop);
     mysprintf(mLOC,"%s",Main.BBSLoc);
     mysprintf(mPATH,"%s",Main.USERPath);                
     mysprintf(mDPATH,"%s",Main.DNPath);                
     mysprintf(mUPATH,"%s",Main.ULPath);                
     
     mNODES = Main.nodes;
     mMINHD = Main.minspace/1000000;                
     mINACt = Main.inac_timeout/60;
     mINACw = Main.inac_warn/60;
...
void LoadNodePrefs(void)
{
	...
	mysprintf(buf, "%sNodePrefs.DAT", path);
    ...
    mysprintf(mSYSPW_tn,"%s",nodeprefs.systempass);
    mysprintf(mNUPE_tn,"%s",nodeprefs.newuserpass);
    mysprintf(mFCHECKDIR_tn,"%s",nodeprefs.fcheck);
    ...
    //Telnet checkmark
    if(nodeprefs.flags&(1<<0)) set(CM_TELNET, MUIA_Selected, TRUE);
    else set(CM_TELNET, MUIA_Selected, FALSE);
     
    //Closed Start
    if(nodeprefs.flags&(1<<1)) set(CM_CLOSED, MUIA_Selected, TRUE);
    else set(CM_CLOSED, MUIA_Selected, FALSE);
    
    //Multi Logins
    if(nodeprefs.flags&(1<<2)) set(CM_MULTI, MUIA_Selected, TRUE);
    else set(CM_MULTI, MUIA_Selected, FALSE);
    
    //Inactiviti disabled ?
    if(nodeprefs.flags&(1<<3)) set(CM_IDLE, MUIA_Selected, TRUE);
    else set(CM_IDLE, MUIA_Selected, FALSE);
    
    DoMethod(thisTEXT,MUIM_SetAsString, MUIA_Text_Contents,"-> Node %ld <-",node);
              
                       
                    
    
}
//Load Node%ld/Doors.DAT
void LoadDoorsConfig(void)
{
     BPTR fhd;
     ...
     mysprintf(buf, "SX:Node%ld/Prefs/Doors.DAT", node);
	fhd = Open(buf, MODE_OLDFILE);
	
     if(fhd)
	{
		Read(fhd, &doors, sizeof(struct DoorsStruct));
		Close(fhd);
     }
     
     mysprintf(mCHAT_door , "%s", doors.chatdoor);
     mysprintf(mMSGED_door, "%s", doors.msgdoor);
     mysprintf(mMSGRE_door, "%s", doors.msgdoorreply);
     mysprintf(mFRONT_door, "%s", doors.frontdoor);
     mysprintf(mNEWF_door , "%s", doors.nfdoor);
     
     mCHATTYP_door = doors.chattype;
     mMSGEDTYP_door= doors.msgtype;
     mMSGRETYP_door= doors.msgdoorreplytype;
     mFRONTTYP_door= doors.fronttype;
     mNEWFTYP_door = doors.nftype;     
        
     set(ST_CHAT_door , MUIA_String_Contents, doors.chatdoor);   
     set(ST_MSGED_door, MUIA_String_Contents, doors.msgdoor);   
     set(ST_MSGRE_door, MUIA_String_Contents, doors.msgdoorreply);   
     set(ST_FRONT_door, MUIA_String_Contents, doors.frontdoor);   
     set(ST_NEWF_door , MUIA_String_Contents, doors.nfdoor);   
     set(ST_NEWM_door , MUIA_Disabled, TRUE);
        
     set(CY_CHAT , MUIA_Cycle_Active, doors.chattype-1);    
     set(CY_MSGED, MUIA_Cycle_Active, doors.msgtype-1);
     set(CY_MSGRE, MUIA_Cycle_Active, doors.msgdoorreplytype-1);
     set(CY_FRONT, MUIA_Cycle_Active, doors.fronttype-1);
     set(CY_NEWF , MUIA_Cycle_Active, doors.nftype-1);    
     set(CY_NEWM , MUIA_Disabled, TRUE);
     
     DoMethod(doorTEXT, MUIM_SetAsString, MUIA_Text_Contents,"-> Node \0334%ld \033n<-",node);
     set(WI_Doors, MUIA_Window_Open,TRUE);
...
void LoadSerial(void)
{
     ...
     fhs = Open(buf, MODE_OLDFILE);
     if(fhs)
     {
        Read(fhs, &Serial, sizeof(struct SerialStruct));
        Close(fhs);
     }   
     
     mysprintf(mSERDEV_sm, "%s" , Serial.device);
     mysprintf(mSERUNIT_sm,"%ld", Serial.unit);
     
     mysprintf(mSERBAUD_sm,"%ld", Serial.minrate);
     mysprintf(mDCERATE_sm,"%ld", Serial.dcerate);
          
     mysprintf(mCONNECT_sm,"%s" , Serial.connect);
     mysprintf(mRING_sm   ,"%s" , Serial.ring);
     mysprintf(mANSWER_sm ,"%s" , Serial.answer);
     mysprintf(mINIT_sm   ,"%s" , Serial.initstr);
        
             
     //set Serial windows Gadgeds start here
     set(POP_SERDEV, MUIA_String_Contents ,mSERDEV_sm);
     set(ST_SERUNIT, MUIA_String_Contents ,mSERUNIT_sm);
     set(ST_MINBAUD, MUIA_String_Contents ,mSERBAUD_sm);
     set(ST_DCE    , MUIA_String_Contents ,mDCERATE_sm);

     set(ST_CONNECT, MUIA_String_Contents ,mCONNECT_sm); 
     set(ST_RING   , MUIA_String_Contents ,mRING_sm);
     set(ST_ATA    , MUIA_String_Contents ,mANSWER_sm);
     set(ST_INIT   , MUIA_String_Contents ,mINIT_sm);
      
     if(Serial.sevenwire!=NULL) set(CM_RTS7,MUIA_Selected, TRUE);          
     else set(CM_RTS7, MUIA_Selected, FALSE);
         
     if(Serial.shared!=NULL) set(CM_SHARED, MUIA_Selected, TRUE);
     else set(CM_SHARED, MUIA_Selected, FALSE);
               
     if(Serial.flags&(1<<0)) set(CM_DSR, MUIA_Selected, TRUE);
     else set(CM_DSR, MUIA_Selected, FALSE);
                   
     
     DoMethod(serTEXT, MUIM_SetAsString, MUIA_Text_Contents,"-> Node \0334%ld \033n<-",node);
     set(WI_Serial, MUIA_Window_Open,TRUE);
...
void LoadProtocol(void)
{
     ...
     DoMethod(WI_PCol, MUIM_CallHook, &ProtoListHook); 
     
     mysprintf(buf,"%sProtocols.DAT", path);
     fhp = Open(buf, MODE_OLDFILE);        
     
     if(!fhp) strcpy(proto.name, "*new*");
     else   
     {
        Read(fhp, &proto, sizeof(struct ProtoStruct));
        Close(fhp);     
        
        strcpy(mNAME_pr  , proto.name);
     
        //mysprintf(mLETTER_pr, "%c", proto.letter);
    
        strcpy(mDEV_pr   , proto.library);
     
        strcpy(mINIT_pr  , proto.initstr);
        
        //mysprintf(buf,"%s"   , proto.efficiency);
        //mEFF_pr = atoi(buf);
         
     }
     
     set(ST_NAME_pr   , MUIA_String_Contents, mNAME_pr);
     set(POP_LIB_pr   , MUIA_String_Contents, mDEV_pr);
         
     //set(ST_LETTER_pr , MUIA_String_Contents, proto.letter);
    
     set(ST_INIT_pr   , MUIA_String_Contents, mINIT_pr);
     
     //set(EFF_pr , MUIA_Slider_Level, mEFF_pr);
          
     DoMethod(protoTEXT, MUIM_SetAsString, MUIA_Text_Contents,"-> Node \0334%ld \033n<-",node);
     set(WI_PCol, MUIA_Window_Open,TRUE);
